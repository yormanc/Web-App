...
public class AdministradorDatos {
    ...
    static {
        String rutaBase = System.getProperty("catalina.base");
        if (rutaBase == null) {
            rutaBase = ".";
        }
        RUTA_ARCHIVOS = rutaBase + "/webapps/WebPerfilDev-1.0-SNAPSHOT/data/";
        System.out.println("✓ Ruta de datos: " + RUTA_ARCHIVOS);
    }
    
    // ===== INICIALIZAR ARCHIVOS POR DEFECTO =====
    public static void inicializarArchivosJSONPorDefecto() {
        try {
            ...
            // Si no existen los archivos, crear directorio
            Files.createDirectories(Paths.get(RUTA_ARCHIVOS));
            
            // Crear archivo usuario si no existe
            if (!Files.exists(Paths.get(rutaUsuario))) {
                String jsonUsuario = "{\"nombreCompleto\":\"Mi Nombre\",\"descripcionPersonal\":\"Desarrollador Web\",\"correoContacto\":\"email@ejemplo.com\",\"numeroTelefono\":\"+57 3001234567\",\"experienciaAnosText\":\"3 años\",\"urlFotoPerfil\":\"https://ui-avatars.com/api/?name=MiNombre&background=667eea&color=fff&size=200\"}";
                Files.write(Paths.get(rutaUsuario), jsonUsuario.getBytes());
            ...
            if (!Files.exists(Paths.get(rutaCompetencias))) {
                ...
                Files.write(Paths.get(rutaCompetencias), jsonCompetencias.getBytes());
    ...
    public static Usuario cargarDatosUsuario() {
        try {
            ...
            e.printStackTrace();
            return new Usuario("Usuario Demo", "Error al cargar datos", "email@ejemplo.com", "+57 3001234567", "3 años", "https://ui-avatars.com/api/?name=Usuario&background=667eea&color=fff&size=200");
        }
    }
    ...
    public static void guardarDatosUsuario(Usuario usuario) {
        try {
            ...
            Files.write(Paths.get(rutaCompleta), json.getBytes());
    ...
    public static void guardarCompetencias(List<Competencia> competencias) {
        try {
            ...
            json.append("]");
            Files.write(Paths.get(rutaCompleta), json.toString().getBytes());
    ...
    private static String extraerValor(String json, String clave) {
        try {
            ...
            System.out.println("Error extrayendo valor para clave: " + clave);
        }
        return "";
    }
    ...
    private static String extraerNumero(String json, String clave) {
        try {
            ...
            System.out.println("Error extrayendo número para clave: " + clave);
        }
        return "0";
    }
